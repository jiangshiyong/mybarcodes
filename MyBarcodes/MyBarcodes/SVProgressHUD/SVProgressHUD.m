...
+ (void)showSuccessWithStatus:(NSString*)string withDuration:(NSTimeInterval)durationTime {

    [[self sharedView] showImage:[[self sharedView] hudSuccessImage] status:string duration:durationTime];
}

+ (void)showErrorWithStatus:(NSString *)string withDuration:(NSTimeInterval)durationTime {
    
    [[self sharedView] showImage:[[self sharedView] hudErrorImage] status:string duration:durationTime];
}
...
- (NSTimeInterval)displayDurationForString:(NSString*)string {
    return MIN((float)string.length*0.06 + 0.7, 5.0);
}
